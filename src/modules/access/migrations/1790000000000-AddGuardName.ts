...
// Tambah kolom `guard_name` (jalur auth: 'web' / 'api') ke roles & permissions.
...
export class AddGuardName1790000000000 implements MigrationInterface {
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        for (const table of ["roles", "permissions"]) {
            await queryRunner.dropIndex(table, `${table}__guard`);
            await queryRunner.dropColumn(table, "guard_name");
        }
    }
}

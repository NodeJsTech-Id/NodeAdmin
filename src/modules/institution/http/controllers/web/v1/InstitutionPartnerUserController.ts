...
export default class InstitutionPartnerUserController {
    ...
    public async index(req: Request, res: Response) {
        ...
        res.render(path.resolve(Module.path, 'views/partner/institution_user/index'), {
            filter,
            datas,
            paginate_data,
            institution,
    ...
    public async create(req: Request, res: Response) {
        ...
        res.render(path.resolve(Module.path, 'views/partner/institution_user/create'), {
            users,
            institution,
            layout: './layouts/main'
        })
    ...
    public async store(req: Request, res: Response) {
        try {
            ...
            res.redirect('/admin/v1/institution/partner/'+req.params.institution_id+'/user')
        } catch (err: any) {
            req.session.flashMessage = { key: 'error', message: err.message }
            req.session.old = req.body
            res.redirect('/admin/v1/institution/partner/'+req.params.institution_id+'/user/create')
        }
    }

    public async edit(req: Request, res: Response) {
        const {data,institution} = await this.institutionUserService.edit(req.params.institution_id, req.params.id)
        res.render(path.resolve(Module.path, 'views/partner/institution_user/edit'), {
            data,
            institution,
            layout: './layouts/main'
        })
    ...
    public async update(req: Request, res: Response) {
        try {
            ...
            req.session.flashMessage = { key: 'error', message: err.message }
            req.session.old = req.body
            res.redirect('/admin/v1/institution/partner/'+req.params.institution_id+'/user/'+req.params.id+'/edit')
        }
    }

...
export default class NewsCategoryService {
	...
	public async index(filter: any) {
		...
		let query = this.newsCategoryRepository.createQueryBuilder('categories')
	...
	public async store(request: any) {
		try {
			const find = await this.newsCategoryRepository.findOne({ where: { name: request.name } })
			if (find) {
				throw new Error("Category Already Exists")
			}
			request = functions.removeEmptyFields(request)
			const data = this.newsCategoryRepository.create({ ...request })
			const result = await this.newsCategoryRepository.save(data)
			if (!result) {
	...
	public async edit(id: string) {
		const data = await this.newsCategoryRepository.findOne({ where: { id } })
		return data
	}

	public async update(id: string, request: any) {
		try {
			const find = await this.newsCategoryRepository.findOne({ where: { id: Not(id), name: request.name } })
			if (find) {
				throw new Error("Category Already Exists")
			}
			...
			request = functions.removeEmptyFields(request)
			const data = this.newsCategoryRepository.merge(category, { ...request })
			const result = await this.newsCategoryRepository.save(data)
			if (!result) {
	...
	public async delete(id: string) {
		...
		const result = await this.newsCategoryRepository.remove(data)
		if (!result) {
			return false
		}
		return result
